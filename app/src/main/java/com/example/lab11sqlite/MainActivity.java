    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_insert.setOnClickListener(view -> {
                    if(ed_book.length() < 1 || ed_price.length() <1)
                        Toast.makeText(MainActivity.this, "Please do not leave the field empty", Toast.LENGTH_SHORT).show();
                    else {
                        try{
                            dbrw.execSQL("INSERT INTO myTable(book, price) values(?,?)",
                                    new Object[]{ed_book.getText().toString(), ed_price.getText().toString()});
                            Toast.makeText(MainActivity.this, "Add book title" +ed_book.getText().toString()+
            ...
            if(ed_book.length() < 1 || ed_price.length() <1)
                ...
                try{
                    ...
                    + "WHERE book LIKE '" + ed_book.getText().toString() +
                                    "'");
                    Toast.makeText(MainActivity.this, "Update book title" +ed_book.getText().toString()+
                            "   Price"+ ed_price.getText().toString(), Toast.LENGTH_SHORT).show();
                    ed_book.setText("");
                    ed_price.setText("");
                } catch (Exception e)
        ...
        btn_delete.setOnClickListener(view -> {
            if(ed_book.length() < 1)
                Toast.makeText(MainActivity.this, "Please do not leave the book title empty", Toast.LENGTH_SHORT).show();
            else {
                try{
                    dbrw.execSQL("DELETE FROM myTable WHERE book LIKE'" + ed_book.getText().toString() + "'");
                    Toast.makeText(MainActivity.this, "Delete book title"
        ...
        btn_query.setOnClickListener(view -> {
            ...
            c.moveToFirst();
            items.clear();
            Toast.makeText(MainActivity.this, "Total"+ c.getCount()+"Pen", Toast.LENGTH_SHORT).show();
            for(int i=0; i<c.getCount();i++)
            {
                items.add("Books:"+c.getString(0)+ "\t\t\t\tPrice:"+c.getString(1));
                c.moveToNext();
            }
